...
var summary = `
  ...
  Example use: jsxtsxscanner -I -L -t 20 --ignore "test" ./path/to/src
`;

program
  .version(require("../package.json").version)
  .usage("[options] <paths ...>\n\n" + summary)
  .option("-t, --threshold <number>", "number of nodes (default: 30)", parseInt)
  .option(
    "-m, --min-instances <number>",
    "min instances for a match (default: 2)",
    parseInt
  )
  .option(
    "-c, --config [config]",
    "path to config file (default: .jsxtsxscannerrc)",
    ".jsxtsxscannerrc"
  )
  .option("-r, --reporter [default|json|pmd]", "specify the reporter to use")
